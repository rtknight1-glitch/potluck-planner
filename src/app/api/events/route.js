...
export async function POST(req) {
  ...
  // Ensure slug uniqueness (very unlikely to collide, but check anyway)
  ...
  const itemRows = (items || [])
    .map((it) => (typeof it === "string" ? { name: it, neededQty: null } : it))
    .filter((it) => it && it.name && it.name.trim())
    .map((it, i) => ({
      event_id: event.id,
      name: it.name.trim(),
      needed_qty: it.neededQty ? Number(it.neededQty) : null,
      sort_order: i,
    }));
